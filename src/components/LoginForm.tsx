...
function LoginForm({ t, locale }: { t: any; locale: Locale }) {
  ...
  return (
    <div className="flex justify-between items-center flex-col">
      <div className="font-bold text-lg">{t?.["signin.text"]}</div>
      <form className="m-10 w-full space-y-5" onSubmit={loginUser}>
        ...
        <div className="text-center text-sm mt-3">
          {t?.["no-account.text"]}
          <Link
            className="text-sm text-blue-500 mr-1 ml-1"
            href={`/${locale}/authenticate/register`}
          >
            {t?.["register.link"]}
          </Link>
        </div>
